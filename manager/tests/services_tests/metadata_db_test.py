...
class MetadataDBServiceTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_tx_commit_and_rollback(self):
        # INSERT and commit this.
        with tx() as conn, conn.cursor() as cur:
            cur.execute("INSERT INTO databases(name) VALUES (%s)", ("database1",))

        # INSERT and ROLLBACK (manually raise exception)
        try:
            with tx() as conn, conn.cursor() as cur:
                cur.execute("INSERT INTO databases(name) VALUES (%s)", ("database2",))
                raise RuntimeError("force rollback")
        except RuntimeError:
            pass

        # Except table storage just "database1" 
        with tx(readonly=True) as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT name FROM databases ORDER BY id;")
            values = [r["name"] for r in cur.fetchall()]

        self.assertEqual(["database1"], values)

    def test_connection_reuse_from_pool(self):
        pool = get_pool()
        conn1 = pool.getconn()
        pool.putconn(conn1)
        conn2 = pool.getconn()
